...
export function CategoryExperience() {
  const [snapshotId, setSnapshotId] = useState(snapshots.at(-1)?.id ?? snapshots[0].id);
  const [expanded, setExpanded] = useState<string | null>("the-odyssey");
  const [ranking, setRanking] = useState(initialRanking);
  const [watched, setWatched] = useState<string[]>(["the-odyssey", "project-hail-mary"]);
  const [saved, setSaved] = useState(false);

  const snapshot = snapshots.find((item) => item.id === snapshotId) ?? snapshots.at(-1)!;
  ...
  return (
    <>
      <section className="snapshot-panel" aria-labelledby="snapshot-title">
        ...
        <div className="snapshot-selector" role="group" aria-label="Elegir corte temporal">
          {snapshots.map((item) => (
            <button
            ...
            >
              ...
              <small>{item.sourceCount} {item.sourceCount === 1 ? "lista" : "listas"}</small>
            </button>
          ))}
        </div>
      ...
      <section className="leaderboard-section" aria-labelledby="leaderboard-title">
        <div className="section-heading split-heading">
          <div>
            ...
            <h2 id="leaderboard-title">
              {snapshot.isConsensus ? "Consenso de nominación" : "Señal editorial"}
            </h2>
          </div>
          <p>
        ...
        <div className="leaderboard">
          ...
          {snapshot.ranking.map((item, index) => {
            ...
            return (
              <div className={`leaderboard-item ${isOpen ? "open" : ""}`} key={item.id}>
                <div className="leaderboard-row">
                  <span className="leaderboard-rank">{String(index + 1).padStart(2, "0")}</span>
                  <div className="leaderboard-title">
                    {item.id === "the-odyssey" ? (
                      <Link href="/peliculas/the-odyssey">{item.title}</Link>
                    ) : (
                      <strong>{item.title}</strong>
                    )}
                    <small>{isCurrent && candidate ? `Posición media ${candidate.average}` : snapshot.date}</small>
                  </div>
                  <div className="coverage-cell">
                    <span>{isCurrent && candidate ? candidate.coverage : `corte ${snapshot.sourceCount}`}</span>
                    <div className="coverage-dots" aria-hidden="true">
                      {[0, 1, 2, 3].map((dot) => (
                        <i
                          className={dot < (isCurrent && candidate ? candidate.sources.length : snapshot.sourceCount) ? "filled" : ""}
                          key={dot}
                        />
                      ))}
                    </div>
                  ...
                  <div className="points-cell">
                    ...
                    <div className="micro-bar" aria-label={`${formatScore(item.score)} puntos de 100`}>
                ...
                {isOpen && candidate ? (
                  <div className="source-breakdown">
                    <div>
                      <p className="section-index">FUENTES QUE SOSTIENEN ESTA POSICIÓN</p>
                      <p>
                        {candidate.firsts} primeras posiciones · mediana {candidate.median} ·{" "}
      ...
      <section className="ranking-lab" id="mi-ranking" aria-labelledby="ranking-title">
        <div className="ranking-intro">
          ...
          <div className="ranking-key">
            <span><i className="status-dot green" /> Vista</span>
            <span><i className="status-dot gray" /> No indicada</span>
          </div>
        </div>
        ...
        <div className="ranking-editor">
            ...
            {rankedCandidates.map((candidate, index) => {
              ...
              return (
                <li key={candidate.id}>
                  ...
                  <div className={`ranking-swatch poster-${candidate.tone}`} aria-hidden="true" />
          ...
          <div className="ranking-save-row">
            ...
            <span aria-live="polite">{saved ? "Simulación guardada en esta vista." : "Sin cuenta · no persistente"}</span>
          </div>
        </div>
      </section>
    </>
